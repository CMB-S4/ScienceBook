...
$20\%$ of the energy is deposited in the plasma \cite{Madhavacheril:2013cna}. CMB-Stage IV is expected to tighten these constraints by a factor of $2$ to $3$ for $f_{\rm sky}=[0.4-0.6]$ \cite{Wu:2014hta}. 
...
\begin{figure}[htbp!]
$\begin{array}{ll}
\includegraphics[width=0.6\columnwidth]{DarkEnergy/ClsPP_ETHOS_a_n.pdf}& 
\includegraphics[width=0.6\columnwidth]{DarkEnergy/ClsPP_ETHOS_f_int.pdf}
\end{array}$
\begin{center}
\caption{\textbf{Left panel}: Fractional difference of the CMB lensing spectrum between a standard $\Lambda$CDM model (with massless neutrinos) and four different ETHOS models with opacity coefficients $a_n$ given in the legend. In all models shown, $5\%$ of the dark matter is allowed to interact with dark radiation. For comparison, we also display a standard massive neutrino model with $\sum m_\nu =0.06$ meV. \textbf{Lower panel}: Similar to the top panel, but we now vary the fraction of dark matter that can interact with dark radiation, for a fixed opacity coefficient of $a_4 = 3\times 10^2$ Mpc$^{-1}$. }\label{fig:Cls_phi_PIDM}
\end{center}
\end{figure}
%
