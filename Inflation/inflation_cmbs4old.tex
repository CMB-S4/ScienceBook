...
{\bf We are not looking for feedback on this section at this time. A lot of forecasting work is underway that we intend to distill down into a first draft of this section by June 15th.}

Figures~\ref{fig_rforecast1} and~\ref{fig_rforecast2} are place-holder forecast done by Victor Buza with Colin Bischoff and John Kovac. There are many caveats that should get mentioned here. For example, the foreground levels assumed are for those in the BICEP2 patch, so we expect more realistic forecasts to be worse for $f_{\rm sky} > 0.1$. For details of the calculation, see \href{http://users.physics.harvard.edu/~buza/20150331_fisher/}{this posting.}

We anticipate a result of this forecasting work will be a recommendation for two different surveys, a deeper smaller-area survey and a shallower broader survey. The first will itself be divided into a low-resolution degree-scale survey targeting the tensor-to-scalar ration, and a high-resolution lensing survey for de-lensing the degree-scale survey. 
