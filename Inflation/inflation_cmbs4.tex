...
The density perturbations are close to scale invariant but not exactly so. In the context of $\Lambda$CDM, {\it Planck} has measured the scalar spectral index to be $n_s=0.9677\pm0.0060$ and has established $n_s-1<0$ at just above $5\,\sigma$. CMB-S4 will improve current constraints on the spectral index by more than a factor two to $\sigma(n_s)=0.0028$, and will provide interesting constraints on the space of inflationary models. 
...
\begin{equation}
\langle\zeta(\vec{k}_1,\vec{k}_2,\vec{k}_3)\rangle=(2\pi)^3\delta(\vec{k}_1+\vec{k}_2+\vec{k}_3)B(k_1,k_2,k_3)\,.
\end{equation}
Invariance under rotations and translations guarantees that the bispectrum $B(k_1,k_2,k_3)$ only depends on the magnitudes $k_1$, $k_2$, and $k_3$. 
