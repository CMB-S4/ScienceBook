...
This connection holds within the simplest models of inflation, but in non-minimal models with additional 
sectors coupled to the inflaton there is a question of whether Eq.~(\ref{eq:Vofr}) is robust.
In fact, it has been shown that excitations  and particle production associated with other fields during inflation can lead to additional sources of
primordial gravity waves~\cite{Cook:2011hg,Senatore:2011sp,Barnaby:2012xt}. Whether such signals can be 
competitive with inflationary vacuum fluctuations -- and thus undermine the connection implied by Eq.~(\ref{eq:Vofr}) -- relies on 
whether proposed models can avoid existing constraints imposed by the scalar power spectrum, non-gaussianity, and self-consistency of the inflationary dynamics
\cite{Barnaby:2012xt,Meerburg:2012id,Ferreira:2014zia,Mirbabayi:2014jqa,Ozsoy:2014sba}. It has been found that in cases where additional sectors are directly coupled to the
inflaton (with stronger than gravitational strength couplings) that it is not possible to avoid existing constraints from Planck, and even for gravitational strength 
couplings there is a tension with the 2015 Planck data \cite{Ozsoy:2014sba,Mirbabayi:2014jqa}. 
...
A possible exception is provided by an inflationary sector, which is gravitationally coupled to a hidden sector containing a light pseudo-scalar and a gauge field during inflation \cite{Barnaby:2012xt,Peloso:2016gqs}. In order to avoid existing constraints, fluctuations of the light scalar must excite fluctuations of the gauge field, which in-turn leads to gravitational wave production. Accounting for the most recent constraints from Planck, it was found in \cite{Peloso:2016gqs} that there exists a range of parameter values which can lead to a gravity wave signal competitive with the vacuum fluctuations while remaining consistent with existing Planck data. For example, the gravitational waves from gauge field production could be measured at a level of $r=10^{-1}$ with a vacuum contribution of only $r=10^{-4}$. While in that case the determination of the scale of inflation is affected by less than one order of magnitude, adjusting the parameters of the scenario may allow for more dramatic modifications of Eq.~(\ref{eq:Vofr}). Fortunately, this scenario predicts a gravity wave spectrum that differs from that of the vacuum fluctuations in several ways. In particular, the gravity waves resulting from the gauge field come with a definite handedness \cite{Anber:2006xt,Sorbo:2011rz} and to evade non-Gaussianity constraints the source field's potential can be tuned so that the production of $\chi$ quanta occurs only around the time the modes contributing to the multipoles relevant for the B-mode search leave the horizon \cite{Namba:2015gja}the B-modes will only be generated at the time gauge field fluctuations leave the Hubble radius \cite{Namba:2015gja}. The latter was an important property that was needed to avoid constraints from the scalar power spectrum, but this also leads to a set of important observation differences from the vacuum fluctuations. In addition, since the signal would be parity violating, the angular bispectrum of B-modes would be dominated by $\ell_1+\ell_2+\ell_3=$even, which would vanish in any theory that respects parity.Thus, the counterexample provides a way to observationally distinguish this mechanism from that of the vacuum fluctuations \cite{Namba:2015gja}.
...
%% Renee Hlozek moved all text on non-Gaussianity up to birefringence to a separate chapter
