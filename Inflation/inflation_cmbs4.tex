...
Correlators including at least one $B$ mode will benefit from the improved sensetivity of $B$ modes significantly. We define \cite{Meerburg2016}
\begin{equation}
...
\caption{Left: forecasts on the constraining power of CMB-S4 on 4 different types of non-Gaussianities (placeholder, orthogonal and equilateral should follow) as a function of $\ell_{\rm max}$. Right: Sensitivity of different types of non-Gaussianities on the minimum multipole. }
...
Measurements of CMB temperature/polarization power spectra indicate that the primordial initial conditions are adiabatic, which means that the initial entropy fluctuation vanishes, that is
\begin{align}
S_{i \gamma}\equiv \frac{\delta n_{i}}{n_{i}}-\frac{\delta n_{\gamma}}{n_{\gamma}} =0.\end{align} The species label $i$ can denote baryons, cold dark matter, or neutrinos. Number densities are denoted by $n_{i}$ and perturbations in them by $\delta n_{i}$.

Adiabatic perturbations are produced in models where the initial perturbations in all species are seeded by   fluctuations of the inflaton. If fluctuations are also sourced by a second field, the initial conditions will be a mixture of adiabatic and entropy (a.k.a isocurvature) perturbations, for which $S_{i\gamma}\neq 0$. These initial conditions determine the acoustic peak structure and large-scale amplitude of CMB anisotropies, as well as large-scale structure statistics \cite{Bond:1984fp,Kodama:1986fg,Kodama:1986ud,Hu:1994jd,Moodley:2004nz,Bean:2006qz}. Observations can thus probe the number of fields during inflation. 
\label{sec:isosec}
Each species can carry isocurvature perturbations in its density (e.g. Refs~\cite{Bucher:1999re,Bucher:2004an,Moodley:2004nz}).\footnote{Neutrinos can also carry velocity isocurvature, but this mode is hard to excite in inflationary models and less well motivated.} Indeed, the normal modes of the perturbation evolution equations correspond to adiabatic, cold dark matter density isocurvature (CDI), baryon density isocurvature (BDI), neutrino density isocurvature (NDI), and neutrino velocity isocurvature (NVI) initial conditions. 
...
Data from WMAP \cite{dunkley09}, \textit{Planck} \cite{Ade:2015lrj}, and ground-based experiments \cite{Enqvist:2000hp,MacTavish:2005yk} indicate that perturbations are predominantly adiabatic. The limits can stated in terms of the fractional primordial power in each isocurvature mode:\begin{equation}
\beta\equiv \frac{P_{S_{i\gamma}}(k)}{P_{S_{i\gamma}}(k)+P_{\zeta\zeta}(k)}.
\end{equation}
The current CMB limits (allowing one isocurvature mode at a time) are shown in Table \ref{table:fisher_model_independent}, along with a Fisher forecast of CMB-S4 sensitivity to isocurvature modes. For the neutrino limits, the spectral slope and correlation (with adiabatic modes) of the isocurvature are marginalized over. In Table. \ref{table:fisher_model_independent}, ``correlated'' refers to totally correlated or anti-correlated (with $\zeta$) isocurvature perturbations. All results are quoted at a fiducial $k=0.05~{\rm Mpc}^{-1}$. Limits to BDI perturbations are not separately listed, as at linear order they are observationally degenerate with CDI perturbations \cite{Gordon:2002gv,Lewis:2002nc,Lewis:2007kz,Gordon:2009wx}. CMB-S4 could on these limits by as much as an order of magnitude (based on the precise assumptions), as shown by the results of a standard Fisher analysis in Table. \ref{table:fisher_model_independent}. 

%\begin{equation}
%\beta\equiv \frac{P_{S_{i\gamma}}(k)}{P_{S_{i\gamma}}(k)+P_{\zeta\zeta}(k)}\leq \left\{\begin{array}{ll}0.0013~(0.038)&\mbox{CDI correlated, $n_{s}^{\rm iso}=n_s^{\rm adi}$ (CDI uncorrelated, $n_{s}^{\rm iso}=1)$},\\
%0.193&\mbox{NDI},\\
%0.074&\mbox{NVI}.\\
%\end{array}\right. \label{eq:isocurvplancklimits}
%\end{equation} 
...
For the rest of this section we focus on two isocurvature scenarios that augment the inflaton with a second field: axion dark matter and the curvaton model. These models produce uncorrelated and correlated isocurvature pertrubrations, respectively.

\subsubsection{Axion Dark Matter}
...
If the standard QCD axion is all of the DM, axion direct detection experiments can be used in conjunction with CMB-S4 to probe $H_I$ in the range
\begin{equation}
 2.5\times 10^6\lesssim H_I/\text{GeV}\lesssim 4\times 10^9\,\,
\text{(QCD axion + direct detection)} \end{equation}
...
\end{equation}
This applies to ALPs in the mass range $10^{-26}\lesssim m_a/\text{eV}\lesssim 10^{-23}$, where effects on lensing of a 1\% axion fraction can be distinguished from CDM.\footnote{We note that obtaining a 1\% fraction in such ALPs requires $f_a\gg 1.8\times 10^{13}\text{ GeV}$, and so isocurvature in ULA DM is guaranteed.} \emph{Detecting isocurvature and lensing effects from ULAs using CMB-S4 can provide a measurement of $H_I$ complementary to searches for tensor modes.}

\subsubsection{The Curvaton Scenario}

One alternative to single-field models is the curvaton scenario, in which an energetically sub-dominant second field $\sigma$ acquires vacuum fluctuations during inflation, becomes more important [by redshifting as $(1+z)^{3}$] later, and sources $\zeta$ \cite{Mollerach:1989hu,Mukhanov:1990me,Moroi:2001ct,Lyth:2001nq,Lyth:2002my}. Eventually, the curvaton decays. Particle candidates for a curvaton include a sneutrino, a light modulus from string theory, as well as others \cite{Postma:2002et,Kasuya:2003va,Ikegami:2004ve,Mazumdar:2004qv,Allahverdi:2006dr,Papantonopoulos:2006xi,Mazumdar:2010sa,Mazumdar:2011xe}. Depending on whether a species $i$ (or the quantum numbers it carries) are produced by, before, or after curvaton decay, perturbations in $i$ are offset from $\zeta$, leading to isocurvature perturbations, that is \cite{Lyth:2001nq,Lyth:2002my,Gordon:2002gv}
...
\end{eqnarray} where $\zeta_{i}$ is the density perturbation in $i$ on surfaces of constant spatial curvature. The parameter $r_{\rm D}$ is the fractional energy density in the curvaton when it decays. 

The precise mixture of isocurvature modes is determined by whether or not baryon number, lepton number, and cold dark matter are produced before, by, or after curvaton decay. Curvaton-type isocurvature is distinct from axion-type isocurvature in the fact that it is correlated (or anti-correlated) with the adiabatic mode. If lepton number is produced by curvaton decay, the lepton chemical potential $\xi_{\rm lep}$ is important in setting the amplitude of NDI modes \cite{Lyth:2002my,Gordon:2003hw,DiValentino:2011sv}:\footnote{Going forward, we restrict our attention to NDI perturbations, as NVI modes are hard to excite in inflationary models.}
\begin{equation}
S_{\nu \gamma}=
-\frac{135}{7}\left(\frac{\xi_{\rm lep}}{\pi}\right)^2\zeta_{\gamma}.\end{equation}

All together, there are $27$ distinct curvaton decay scenarios, as baryon number, lepton number, and cold dark matter could each be produced before, by, or after curvaton decay. Viable models are those in which one of baryon number or cold dark matter is produced by curvaton decay, as well as those in which \textit{both} baryon number and cold dark matter are produced after curvaton decay. We demark the models with the notation ($b_{x}$, $c_{y}$, $L_{z}$), where $x\in ({\rm before, by, after})$, $b$ denotes baryon number, $c$ denotes cold dark matter, and $L$ denotes lepton number. For example, $(b_{\rm before}, c_{\rm by}, L_{\rm by})$ labels a model in which baryon number is produced before curvaton decay, cold dark matter by curvaton decay, and lepton number by curvaton decay.

Current isocurvature limits favor values of $r_{\rm D}\simeq 1$, except for models in which baryon number is produced by curvaton decay and CDM before (or vice versa), which favor central values of $r_{\rm D} \simeq 0.16$ ($r_{\rm D} \simeq 0.84$). These numerical values are driven by the requirement that isocurvature nearly vanish.

The current limits \cite{Smith/Grin:2015} are shown in Table \ref{limits_rd}, along with a Fisher forecast of the sensitivity of CMB-S4 to curvaton parameters from isocurvature. 
\begin{table}[htbp]
\begin{center}
\begin{tabular}
{lcc}\hline \hline {\rm scenario} &  $ \Delta r_D/r_{D}^{\rm adi}$ (\emph{Planck})&$ \Delta r_D/r_{D}^{\rm adi}$(CMB-S4)\\ 
\hline \\
$(b_{\rm by},c_{\rm before},L_{y_{L}})$ & $0.03$&$XX$\\
$(b_{\rm before},c_{\rm by},L_{y_{L}})$ &  $0.01$ &$XX$\\
$(b_{\rm by},c_{\rm after},L_{y_{L}})$ &  $0.04$&$XX$\\
$(b_{\rm after},c_{\rm by},L_{y_{L}})$ & $0.008$&$XX$\\
$(b_{\rm by},c_{\rm by},L_{y_{L}})$ &  $0.007$&$XX$\\
$(b_{\rm after},c_{\rm after},L_{y_{L}})$ & $0.003$&$XX$\\
\\ \hline \hline 
\end{tabular}
...
\hline \\
$(b_{\rm by},c_{\rm before},L_{\rm by})$ &$XX$ &$XX$\\
$(b_{\rm before},c_{\rm by},L_{\rm by})$ &$XX$  & $XX$\\
$(b_{\rm by},c_{\rm after},L_{\rm by})$ &$XX$  &$XX$\\
$(b_{\rm after},c_{\rm by},L_{\rm by})$ & $XX$&$XX$\\
$(b_{\rm by},c_{\rm by},L_{\rm by})$ & $XX$ & $XX$\\
$(b_{\rm after},c_{\rm after},L_{\rm by})$ &$XX$ &$XX$\\
\\ \hline \hline 
\end{tabular}
\caption{Isocurvature constraints on $\xi_{\rm lep}^{2}$ ($95\%$ C.L.) using \textit{Planck} TT+BAO+LowP data \cite{Smith/Grin:2015} in viable curvaton decay-scenarios where lepton number is generated by curvaton decay, and Fisher forecasts for CMB-S4 sensitivity.\textbf{$XX$ in the tables denotes results pending the result of a final Fisher analysis based on parameters developed by the forecasting group.}}
\label{limits_xilep}
\end{center}
\end{table}


Depending on the precise scenario, a standard Fisher analysis shows that the S4 sensitivity to curvaton-sourced isocurvature should improve by a factor of $2-4$ on current limits. In models with nearly-canceling CDM and baryon isocurvature perturbations, S4 limits to neutrino isocurvature drive an improvement in the sensitivity to the lepton asymmetry from $\Delta \xi_{\rm lep}^{2}\simeq 0.015$ to $\Delta \xi_{\rm lep}^{2}\simeq 0.003$. This dramatic improvement would make CMB limits comparably sensitive to BBN probes of lepton asymmetry generated in the curvaton model.
...
\end{equation} Curvaton-generated CIPs are proportional to the adiabatic curvature perturbation $\zeta$, with $S_{\rm bc}=A\zeta$, where $A\simeq 17$ [$A\simeq -3$] in the $(b_{\rm by}, c_{\rm before}, L_{\rm z})$ [$(b_{\rm before}, c_{\rm by}, L_{\rm z})$] scenario. In this scenario, the initial relative densities of baryons and dark matter vary, but with no additional overall or radiation density. 

CIPs are relatively unconstrained at the level of the CMB power-spectrum, but would induce non-Gaussianities in the CMB \cite{Grin:2011nk,Grin:2011tf,Grin:2013uya,He:2015msa} due to a spatially varying baryon-photon sound speed at recombination. As with weak gravitational lensing \cite{Hu:2001kj}, the CIP field $\Delta(\hat{n})$ can thus be reconstructed using CMB data. Using a Fisher analysis \cite{He:2015msa}, we find that at S4 sensitivity, $A$ at the threshold for a $95\%$ C.L. detection is $A\simeq 10$, and so a CIP test of the $(b_{\rm by}, c_{\rm before}, L_{\rm z})$ scenario is within reach of CMB-S4. This is a vast improvement over the \emph{Planck} sensitivity, which at $95\%$ C.L. is $A\simeq 43$. Uncorrelated CIPs, are less well motivated theoretically. The sensitivity of CMB-S4 to a scale-invariant angular power spectrum of CIPs uncorrelated CIPs is an amplitude of $\Delta_{\rm cl}=0.004$ at the $95\%$ C.L. detection limit, to be compared with the upper limit of $\Delta_{\rm cl}\leq 0.077$ from the WMAP analysis of Ref. \cite{Grin:2013uya}. 
