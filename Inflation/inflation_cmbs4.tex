...
Denoting the number of e-folds before the end of inflation at which the CMB pivot scale exits the horizon as $\mathcal{N}_\star$, the constraints on the spectral index from the {\it Planck} satellite is $\mathcal{O}(1/\mathcal{N}_\star)$. While this could be a coincidence, it seems plausible that 
...
Note that $r= 16\epsilon(\mathcal{N}_\star)$ is naturally larger in classes I and IV than in classes II and III. Upper limits on the amount of primordial gravitational would then favor classes II and III and strongly constrain or exclude I and IV. A detection of the running of the spectral index would allow a measurement of $p$ independent of $\mathcal{N}_\star$, but unfortunately cannot be achieved with CMB-S4. As a consequence, we will discuss the four classes in the absence of a constraint on $\mathcal{N}_\star$ and assuming moderately efficient reheating so that $50<\mathcal{N}_\star<60$. The upper limit represents an approximation to instantaneous reheating, which strictly speaking varies from model to model typically implies $\mathcal{N}_\star\approx60$.

For $|p|\ll1/|\ln\mathcal{N}_{\rm eq}/\mathcal{N}_\star|$ classes I, II, and III become degenerate

Discussion of first class

Discussion of second class

Discussion of third class

Discussion of fourth class
