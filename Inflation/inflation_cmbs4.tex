...
\end{itemize}
If, through complementary observations, we can conclusively determine that any detected signal is dominated by vacuum fluctuations then a detection would also
\begin{itemize}
...
\end{equation}
where $k=\pm1$ allows for curvature and the time evolution is specified by the scale factor, $a(t)$. For much of the rest of the discussion we will assume spatial flatness ($k=0$) because it is consistent with all current data, but we will return to constraints on the curvature in Section xxx. The Hubble parameter, $H=\dot{a}/a$, gives the rate of expansion of the universe. 
...
How model-independent is the relationship (equation $r\leftrightarrow V$)? To answer this question, a significant number of papers has dealt with scenarios of low-scale inflation where $r$ is nevertheless large and observable. These scenarios provide an opportunity for escaping the Lyth bound \cite{Lyth:1996im}. Most of this body of work has used non vacuum sources: some form of matter $\chi$ is created during inflation and acts as a source of gravitational waves \cite{Cook:2011hg,Senatore:2011sp}. The field $\chi$ should be massless, since massive particles contribute a smaller quadrupole moment and are a weaker source of gravitational waves \cite{Barnaby:2012xt}. An important limitation of these scenarios is that $\chi$ should be a stronger source of tensor than of scalar modes~\cite{Barnaby:2012xt,Ferreira:2014zia,Mirbabayi:2014jqa,Ozsoy:2014sba}, since the properties of the latter are tightly constrained by experiment, especially by the non-observation of nongaussianities \cite{Barnaby:2010vf}  but also (in some models), by constraints on the running of $n_s$ \cite{Meerburg:2012id}. As a consequence, it has been  assumed that the dynamics generating $\chi$ should be decoupled from the inflaton sector  \cite{Barnaby:2012xt}. 
...
All constraints from nongaussianities can be finally evaded by assuming that the production of $\chi$ occurs during the time when scales corresponding to $\ell={\cal O}(1)\div {\cal O}(100)$ have left the horizon \cite{Namba:2015gja}. In fact, B-modes associated to primordial tensors are observed only at those scales, whereas nongaussianities are mostly constrained by CMB observations at shorter scales. Reference  \cite{Namba:2015gja} provides an existence proof of a viable model where $r$, that is determined by non vacuum sources, could be measured to be of the order of $10^{-1}$ even if the slow roll parameter $\epsilon$ is $10^{-5}$, so that the vacuum contribution to $r$ is only $10^{-4}$. This model comes with a rich phenomenology: the tensor modes produced this way would be strongly scale dependent (including a blue tilt), would be parity violating (with a $TB$ correlator that might be detectable in a cosmic variance limited experiment) and with a large B-mode bispectrum.

Finally, note that these secondary production mechanisms cannot be used to obtain a signal from a contracting primordial phase for the same reason the vacuum fluctuations are too small to observe: the Hubble parameter is too small. {\bf check this with Marco/Lorenzo}
