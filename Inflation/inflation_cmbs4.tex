...
\begin{itemize}
\item {To add: quantify improvements on scalar power spectrum: running, wiggles}
...
Even such a factor of two improvement on $f_{\rm NL}^{\rm equil}$ would further constrain scenarios with secondary production of gravitational waves of comparable strength to the vacuum signal from fields coupled to the inflaton....

A detection of local non-Gaussianity would have far reaching theoretical implications, since any significant detection of local $f_{\rm NL}$ would rule out all models of single clock inflation [astro-ph/0407059]. In addition, it would open the door to significant cosmic variance on all scales from coupling of fluctuations within our observed volume to any super-Hubble modes \cite{}. 
