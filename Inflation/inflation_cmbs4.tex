...
As shown in the right panel of Fig.~\ref{fig_rforecast1}, if $r$ is as large as 0.01, then larger sky area will be needed to improve precision; thus, the CMB-S4 deep survey must be designed with the flexibility to increase sky area in the event of a detection.
...
The agreement between the map-based approach and the power-spectrum-based approaches provides evidence that the foreground mitigation approach in the design of CMB-S4 (in particular the number of observing bands and their placement in frequency) is adequate.
...
%calculated from bandpower window functions. 
...
\caption{(Top panels)
Forecasted uncertainty on $r$, as a function of $f_\mathrm{sky}$, for an
