...
\end{equation}
If the Hubble rate and slow-roll parameter only weakly depend on time as in slow-roll inflation, these will be $n_{\rm s}(k)\approx 1$ and $n_{\rm t}(k)\approx 0$ and can be expanded around a pivot scale $k_\star$ accessible by the CMB
\begin{equation}
n_{\rm s}(k)-1=n_{\rm s}-1+\left.\frac{dn_{\rm s}(k)}{d\ln k}\right|_{k_\star}\ln(k/k_\star)+\dots\qquad{\rm and}\qquad n_{\rm t}(k)=n_{\rm t}+\left.\frac{dn_{\rm t}(k)}{d\ln k}\right|_{k_\star}\ln(k/k_\star)+\dots
\end{equation}
In this approximation, the power spectra are
\begin{eqnarray}\label{eq:power_spectra_power_law}
\mathcal{P}_{\zeta}(k)&=& A_{\rm s}\left(\frac{k}{k_\star}\right)^{n_{\rm s}-1+\frac{1}{2}\left.\frac{dn_{\rm s}}{d\ln k}\right|_{k=k_\star}\ln(k/k_\star)+\dots}\,,\nonumber\\
\mathcal{P}_{t}(k)&=& A_{\rm t} \left(\frac{k}{k_\star}\right)^{n_{\rm t}+\frac{1}{2}\left.\frac{dn_{\rm t}}{d\ln k}\right|_{k=k_\star}\ln(k/k_\star)+\dots}\,,
\end{eqnarray}
%\begin{equation}\label{eq:specind}
%\mathcal{P}_{\zeta}(k)\equiv A_{\rm s}\left(\frac{k}{k_\star}\right)^{n_{\rm s}-1}\qquad{\rm and}\qquad\mathcal{P}_{t}(k)\equiv A_{\rm t} \left(\frac{k}{k_\star}\right)^{n_{\rm t}}\,,
%\end{equation}
where $A_{\rm s}$, $A_{\rm t}$ are the scalar and tensor amplitudes, and $n_{\rm s}$ and $n_{\rm t}$, are the scalar and tensor spectral index, respectively, both at the pivot scale. 
...
\begin{equation}
C_{BB,\ell}=\int \frac{dk}{k}\mathcal{P}_{\rm t}(k)\left|\int\limits_0^{\tau_0} d\tau S_B^{(T)}(k,\tau)j_\ell(k(\tau_0-\tau))\right|^2\,.
\label{eq:clbb}
\end{equation}
...
%(For a single field with a sound speed less than one, or multiple fields, $n_{\rm t}/r<-1/8$ instead \cite{Price:2014ufa} 
...
%with 
\begin{equation}
B_{\zeta\zeta\gamma_s}(k_1,k_2,k_3)= 16 \pi^4 A_{\rm s}^2 \sqrt{r}f_\mathrm{NL}^{\zeta\zeta\gamma} F(k_1,k_2,k_3).
\end{equation} 
