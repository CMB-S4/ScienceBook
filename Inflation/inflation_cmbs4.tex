...
For class I, constraints from the {\it Planck\/} satellite and the { BICEP}2/{\em Keck Array} experiments~\cite{Ade:2015tva} translate into $p=0.32\pm0.16$ at $1\,\sigma$, and favor models with inefficient reheating. The best-fit point in this class is $r=0.044$ and $n_{\rm s}=0.973$, which is currently disfavored relative to class II at 1--$2\,\sigma$. Upper limits on $r$ directly translate into constraints on $p$. A $1\,\sigma$ upper limit on the amount of primordial gravitational waves from CMB-S4 at a level of $r<0.001$ would imply $p<0.013$ and effectively rule out this class as it degenerates into class II in this limit. 
...
The Fisher projections for {\it Planck\/} around a fiducial model of $f_{10}=0.01$ are $f_{10}<0.032$. This corresponds to $G\mu < 3.5\times 10^{-7}$ at $95\%$~CL and is consistent with the {\it Planck\/} constraints.
