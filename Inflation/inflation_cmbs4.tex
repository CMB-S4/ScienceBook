...
Finally, beyond adjusting the inflationary dynamics and particle content, one can break the above relationship between the $r$ and the scale of inflation by suppose that the tensor  modes (and the scalar modes) do not start in the Bunch-Davies state \cite{Collins:2014yua,Ashoorioon:2014nta}. Doing so generically introduces a stronger scale-dependence into the tensor spectrum \cite{Aravind:2014axa} (and additional non-Gaussianity). In addition, this means of achieving observable tensor modes from a small field range has a similar feature to the large field range models: to be a complete one should show that the initial state is not only acceptable from low energy considerations, but can be generated by pre-inflationary physics.

In summary, a conclusive detection with a CMB S4 instrument would provide strong evidence that the theory of quantum gravity must accommodate a shift symmetry for the inflaton. As discussed in Section \ref{sec:scale-of-inflation}, there may be some room for small-field inflation to generate a signal from non-vacuum fluctuations. Whether or not future observational constraints can completely close that window, the {\it absence} of a detection of B-modes with CMB S4 will mean that the inflationary dynamics that explain our observed scalar fluctuations do not require a large field range. 

{\it A detection of $r$, together with high confidence that the gravitational waves are significantly sourced from vacuum fluctuations, would provide the only data point for the foreseeable future that weighs in on quantum gravity.}
...
%Constraints on the tensor-to-scalar ratio with CMB-S4 would disfavor classes I, II with $\mathcal{N}_{\rm eq}\gtrsim 1$, III, and IV relative to class II with $\mathcal{N}_{\rm eq}\ll 1$ at $2-3\,\sigma$.  
...
To better understand the implications, let us discuss the models that underly the classes favored by current data, classes I and II. The underlying potentials can be obtained from the well-known equations
\begin{equation}
...
{\em In the absence of a detection, CMB-S4 would rule out or disfavor all models that naturally explain the observed value of the scalar spectral index and in which the characteristic scale in field space is the Planck scale.}

Unfortunately, because of the scaling $M\propto \sqrt{\mathcal{N}_{\rm eq}}$ it will only be possible to obtain constraints $M\lesssim M_p$ but not $M\ll M_p$. 
