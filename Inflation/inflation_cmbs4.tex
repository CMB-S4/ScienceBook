...
Fluctuations in the value of the inflaton field, which is responsible for the accelerating expansion of the cosmos, evolve differently, depending on whether their wavelength $\lambda$ is less than or greater than the horizon length $L = c/H$. When $\lambda << L$, the uncertainty principle limits how smooth the field can be. As a result, the amplitude of the fluctuation is inversely proportional to $\lambda$ and thus decreases as the universe expands. (The influence of the uncertainty principle is reflected by the appearance of Planck’s constant ℏ in the expression for the amplitude.) As $\lambda$ becomes larger than the horizon, the crest and trough of the wave cease to be in causal contact, so the amplitude stops evolving. For superhorizon evolution, its asymptotic value corresponds to replacing the wavelength in the subhorizon case with $2\pi L$. Eventually, cosmic expansion stretches the fluctuations to astrophysically large length scales.
}
\label{fig:PTfigs}
\end{figure}
...
The spacetime metric itself, at least in a linearized treatment, presumably obeys the uncertainty principle as well. As a result, we expect a nearly scale-invariant spectrum of gravitational waves
...
An axion is the pseudo-Goldstone boson of a spontaneously broken global $U(1)$ symmetry, known as the Peccei-Quinn (PQ) symmetry. The symmetry is broken when fluctuations (thermal or quantum) become smaller than the PQ symmetry breaking scale, $f_a$ (also known as the axion decay constant). During inflation, this implies that the PQ symmetry is broken if $H_I/2\pi<f_a$. Since $H_I$ is bounded by the current constraint on $r_{0.05}<0.12$ to be $H_I<8.8\times 10^{13}\text{ GeV}$, all axion DM with $f_a>1.8\times 10^{13}\text{ GeV}$ is associated with a PQ symmetry broken during inflation. This accounts for the QCD axion in the ``anthropic'' window (roughly half of the allowed range of $f_a$ on a logarithmic scale), all axions with GUT scale decay constants (such as string axions~\cite{Svrcek:2006yi,Arvanitaki:2009fg}, and those required for the Fuzzy DM scenario of structure formation~\cite{Hu:2000ke,Marsh:2013ywa}), and axions with lower $f_a$ in models of low-scale inflation.

The axion field, $\phi$, has a classical initial displacement, the ``initial misalignment angle,'' imprinted by symmetry breaking. Inflation homogenizes the initial displacement and leads to a spatially uniform axion condensate with initial value $\phi_{i,0}\in [0,2\pi f_a]$. The initial homogeneous value has a random piece from symmetry breaking, and a constant contribution from the average of fluctuations, $\langle\phi_i^2\rangle=\phi_{i,0}^2+(H_I/2\pi)^2$. In this scenario, axion fluctuations have two sources: the usual adiabatic fluctuations which it inherits gravitationally from the radiation bath after reheating, and the quantum fluctuations $\langle\delta\phi^2\rangle$ arising during inflation. The axion field is massless during inflation and the fluctuations have amplitude $\langle\delta\phi^2\rangle = (H_I/2\pi)^2$, with spectral index $n_{\rm iso}=n_T=-2\epsilon$ (e.g. Refs.~\cite{Axenides:1983hj,Fox:2004kb,Hertzberg:2008wr}). 

Upon leaving the horizon during inflation, the quantum fluctuations classicalize as usual, such that in the post-inflation Universe they lead to initial density perturbations in the axion field, with amplitude:
\begin{equation}
A_I = \left(\frac{\Omega_a}{\Omega_d}\right)^2\frac{(H_I/M_{pl})^2}{\pi^2(\phi_i/M_{pl})^2} \, .
\end{equation}
Since the axion is an independent field from the inflaton, not involved in the process of inflation, and furthermore since the axion is sub-dominant to the radiation at initial times, these fluctuations lead to \emph{uncorrelated isocurvature perturbations} in the axion DM. The uncorrelated CDM isocurvature amplitude is bounded by \emph{Planck} as $A_I/A_s<0.038$~\cite{Ade:2015lrj}.

The axion DM relic density is sourced by coherent oscillations of the homogeneous axion field, and is a function of the axion mass, $m_a$, and initial displacement, $\phi_i$~\cite{Preskill:1982cy,Abbott:1982af,Dine:1982ah,Turner:1983he,Steinhardt:1983ia,Marsh:2010wq}. Thus, the isocurvature amplitude, $A_I$, can be recast as a function of $m_a$ and $\Omega_a$. If these can be independently determined (see below) then \emph{in the context of axion DM models, a measurement of the isocurvature amplitude is a measurement of $H_I$.}

The constraint imposed on $H_I$ by iscourvature for the QCD axion is orders of magnitude stronger than that imposed by direct constraints from $r$~\cite{Fox:2004kb,Hertzberg:2008wr,Visinelli:2014twa}. For the QCD axion with $f_a<M_{pl}$, there is no overlap between the regions of parameter space consistent with existing constraints on isocurvature and tensors, and also accessible with S4. Thus, if the PQ symmetry is broken during inflation, QCD axion DM requires a low energy scale of inflation, and implies undetectable tensor modes.\footnote{These conclusions assume single-field, slow-roll inflation and a standard thermal history. They can be avoided in various extended models, e.g. Refs.~\cite{Fairbairn:2014zta}.% ADD MORE REFS
 If the PQ symmetry is broken after inflation, $f_a<H_I/2\pi$, then no isocurvature perturbations are produced, and so we do not consider this scenario further. In this case, axion strings, domain walls, and miniclusters complicate the phenomenology.}
...
Axion direct detection can measure the mass of the QCD axion, if it is all of the DM. Fig.~\ref{fig:qcd_isocurvature} (left panel) shows how axion direct detection experiments can be combined with S4 to measure $H_I$ for low scale inflation. S4 will be sensitive to isocurvature amplitude in the range $0.008<A_I/A_s<0.04$ (where the upper limit is imposed by \emph{Planck}). A detection of axion-type isocurvature with S4, combined with a detection of axion DM by either ADMX~\cite{Asztalos:2009yp} (in operation, approx $10^{12}\lesssim f_a/\text{ GeV}\lesssim 10^{12}$), or CASPEr~\cite{Budker:2013hf} (proposed, approx $10^{15}\lesssim f_a/\text{ GeV}\lesssim M_{pl}$) allows measurement of $H_I$ in the range: 
\begin{equation}
2.5\times 10^6\lesssim H_I/\text{GeV}\lesssim 4\times 10^9\, .
\end{equation}
This range of low-scale inflation is totally inaccessible to searches for tensor modes or non-Gaussianity, making axion isocurvature a very complimentary probe of inflation in combination with DM direct detection.

We now consider ALPs (see e.g. Refs.~\cite{Marsh:2013taa,Marsh:2014qoa}). Axion DM has a number of distinctive features in large scale structure and the CMB (see OUR OTHER SECTION) that could allow for measurement of $(m_a,\Omega_a)$ to extremely high precision using CMB-S4. For an axion-like particle with $10^{-32}\lesssim m_a/\text{eV}\lesssim 10^{-23}$ the density can be determined to less than one percent of the total DM abundance. Fig.~\ref{fig:qcd_isocurvature} (right panel) shows isocurvature constraints possible with S4, compared to tensor constraints, fixing the ALP fraction to 1\%. In contrast to the QCD axion, there are masses for which tensor modes impose a stronger constraint on $H_I$ than isocurvature. There are also regions of overlap between tensor and isocurvature constraints. Using S4 in these regions, it is possible to make a combination measurement of isocurvature and axion parameters, giving an independent measurement of $H_I$ for ALPs in the mass range $10^{-26}\lesssim m_a/\text{eV}\lesssim 10^{-23}$.
