...
\end{equation}
with $M=\sqrt{\alpha(p)\mathcal{N}_{\rm eq}}M_{\rm P}$, where $\alpha(p)=4p/(1-p)^2$. For $p>1$ inflation occurs when $\phi\ll M$. In this regime, the potential behaves like a hilltop model, $V(\phi)\simeq V_0(1-\left(\phi/M\right)^n)$, with $n=2p/(p-1)$. For $0<p<1$ inflation occurs for $\phi\gg M$ and $V(\phi)\simeq V_0(1-\left(M/\phi\right)^n)$ with $n=2p/(1-p)$. In the limit $p\to0$, in which classes I, II, and III become degenerate, the $\phi$-dependence is logarithmic. 
