...
\chapter{$N_{\rm eff}$ and Light Dark-Sector Relics}
...
The cosmic neutrino background is detected at high significance through the measurement of $\Neff$.  In practice, $\Neff$ is just a measurement of the gravitational influence of free streaming radiation that is decoupled from the photon-baryon fluid.  In addition to probing neutrino physics, $\Neff$ is therefore equally a probe of any light dark-sectors.  I this section, we will explore the broad implications of measurements of $\Neff$ and CMB implications for light particle more generally.

CMB-S4 can reach some extremely interesting targets relevant to our cosmological history and implications for particle physics. These are most easily characterized in terms of the change to $\Neff$, $\Delta\Neff$, due to a single additional species that was in thermal equilibrium with the Standard model that decouples at some temperature $T_{\rm freeze}$.  This picture leads to two important theoretical targets that are within reach of CMB Stage IV:
\begin{itemize}
\item $\Delta \Neff \geq 0.047$ are predicted for models containing additional light particles of spin $1/2$ and $1$ that were in thermal equilibrium with the particles of the Standard Model back to the time of reheating.  A CMB experiment reaching $\sigma(\Neff) \lesssim 0.02-0.03$ would be sensitive to all models in this very broad class of extensions of the Standard Model at 2$\sigma$, which includes any thermal population of gravitinos and dark photons.  $\Delta \Neff \lesssim 0.05$ is reaches any thermal relic (including spin 0) that decoupled before the QCD phase transition.  
\item $\Delta \Neff \geq  0.027$ are predicted for models containing additional light particles of any spin $0$, $1/2$ and $1$ that were in thermal equilibrium with the Standard Model.  A CMB experiment reaching $\sigma(\Neff) \lesssim 0.02-0.03$ would be sensitive to all models in this very broad class of extensions of the Standard Model at 1$\sigma$, which includes a wide range of models predicting axions and axion-like particles. 
\end{itemize}
...
Another advance at CMB Stage IV will be a fairly high precision {\it independent} measure of primordial helium, $Y_p$ and $\Neff$. These two quantities probe neutrino and BSM physics in related, but different ways, allowing even finer probes of BSM physics, especially in the neutrino sector.  In in particular, helium gives an integrated measure of this expansion rate convolved with the weak interaction rates that interconvert neutrons and protons. Helium therefore gives us a handle on neutrino energy distribution functions in a way that which $\Neff$ alone does not.  Furthermore, $Y_p$ and $\Neff$ probe the density of radiation at well separated times, a few minutes and 380 000 years respectively, which provides a window non-trivial evolution in the energy density of radiation.  
...
\section{Relativistic Species at Recombination}\label{sec:Neff}
...
These results are tantalizingly close to some exciting theoretical targets.  This raises the question of whether we can reach these targets at higher significant by other means.  First, constraints on $\Neff$ improve significantly if $\ell_{\max}^{TT} > 3000$.  These modes are currently ignored due to foregrounds, but foreground removable could makes some of the modes accessible.  Second, one could also imagine the sensitivity could be improved in combination with large scale structure surveys such as DESI and LSST, both by breaking degeneracies and providing a future measurement of the primordial power spectra.  


\section{Neutrino constraints on BSM physics}\label{sec:BSMneff}
...
Light particles of spin-0 (scalar fields) are highly constrained by naturalness.  In the absence of a symmetry, one would expect them to be heavy and thus a poor candidate for dark radiation.  On the other hand, (pseudo) Goldstone bosons are naturally lights and they appear generically from spontaneous breaking of some high energy global symmetry.  A ubiquitous  example in Beyond the Standard model physics are axions and/or axion-like particles (ALPs).  Axions have been introduced to solve the strong-CP problem~\cite{Peccei:1977hh}, the hierarchy problem~\cite{Graham:2015cka} and the naturalness of inflation~\cite{Freese:1990rb}.  Furthermore, they appear generically in string theory, in large numbers, leading to the qualitative phenomena describe as the string axiverse~\cite{Arvanitaki:2009fg}.  They may even be tied to the origin of the breaking of the flavor, baryon/lepton number symmetries of the standard model.
...
\subsubsection{Constraints on cold axion energy density}
...
\subsubsection{Axion Isocurvature \label{axion_iso}}
...
Like the case of $\Neff$-only forecasts, constraints on $Y_p$ and $\Neff$ are primarily sensitive to $f_{\rm sky}$ as show in Figure~\ref{fig:YpNeff_fsky}.  Although both parameters are sensitive to the damping tail and we also see the significant impact of varying the beamsize from $1'$ to $3'$.  
