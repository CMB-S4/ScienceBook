...
\caption{Relationship between freeze-out temperature ($T_F$) and the axion-photon coupling ($\Lambda_{\gamma}$).  The region in light red illustrates the axion parameter space that predicts $\Delta \Neff = 0.027$ for a plausible reheat temperature of $T_{\rm R} = 10^{10}$ GeV.  Also shown are the existing experimental and astrophysical constraints.  While the freeze-out of thermal axions is independent of the mass, experimental probes of the coupling are strongly mass dependent.  For $T_F < 10^2$~GeV the axion coupling would predict $\Delta \Neff > 0.027$, as qualified by Figure~\ref{fig:Neff_thermal}.  We see that sensitivity to $\Delta \Neff= 0.027$ can be orders or magnitude more sensitive to axions than existing probes.}
