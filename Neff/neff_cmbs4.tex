...
Second, one could also imagine the sensitivity could be improved in combination with large scale structure surveys such as DESI and LSST, both by breaking degeneracies and providing a future measurement of the power spectrum.  
...
\begin{figure}[t!]
\begin{center}
\includegraphics[width=0.65\textwidth]{Neutrinos/Nfsky_lmax.pdf}
...
\begin{figure}[t!]
\begin{center}
\includegraphics[width=0.65\textwidth]{Neff/Neff_lownoise_beam.pdf}
\caption{Forecasts for $\sigma(\Neff)$ assuming futuristic values of the beamize in arcmin (') and temperature noise in $\mu$K-arcmin, assuming $f_{\rm sky} = 0.4$.  These contours are for unlensed spectra with $\ell^{TT}_{\rm max} =3000$ and $\ell_{\rm max} = 8000$ for all other spectra.  Even with these overly optimistic assumptions, we see it is challenging to the threshold $\Delta \Neff =0.027$ at 2$\sigma$, $\sigma(\Neff) = 0.013$, without larger values of $f_{\rm sky}$.  }
\label{fig:Neffbeam}
\end{center}
\end{figure}
