...
\subsubsection{Equation of motion parametrization}

{\bf MR: w0wa here?}

We parametrize deviations of the speed of GWs from the speed of light, with the parameter $c^2_{\rm GW}$. If the effect of primordial GWs on the B-mode polarization of the CMB is detected then the same observations will be capable of constraining their speed at the time of recombination.
In the left panel of Figure \ref{fig:MixedDEG} we show the marginalized joint forecast constraint on the tensor to scalar ratio and the speed of GWs.
As we can see these two parameters are only weakly degenerate.

{\bf MR: uploading tomorrow the same forecast with different r fiducial}

\subsubsection{Theory parametrization}

{\bf MR: beware of inconsistent notation in the figures $f_0 \rightarrow f_{\rm early}$ and  $f_1 \rightarrow f_{\rm late}$. Will be fixed tomorrow.}
...
\hline
CMB-S4 + DESI  & 0.002 & 0.01 & 0.05 & 0.002 & 0.006 & 0.02 & 0.001 \\
\hline
\end{tabular}
\caption{Forecasted $68\%$ C.L. constraints on different models: the trigger parameter $\gamma_{\rm L}$; constant EFT couplings $\Omega_0$, $\gamma_0^{(2)}$ and $\gamma_0^{(3)}$; constant Horndeski couplings $\tilde{M}_0$, $\alpha_{\rm B}$ and $\alpha_{\rm T}$.}
...
\hline
Datasets 			& $\Omega_{\rm early}$ & $\Omega_{\rm late}$ & $\tilde{M}_{\rm early}$ &  $\tilde{M}_{\rm late}$ & $\alpha^B_{\rm early}$ & $\alpha^B_{\rm late}$ & $\alpha^T_{\rm early}$  & $\alpha^T_{\rm late}$   \\
\hline
\hline
...
\hline
CMB-S4 + DESI  &  0.03 &  0.01 & 0.04 & 0.007 & 0.04 & 0.08 & 0.01 & 0.001\\
\hline
\end{tabular}
\caption{Forecasted $68\%$ C.L. constraints on early and late time values of different EFT couplings.}
\label{table:ForecastAlphaAtan}
\end{center}
\end{table}

We consider two parametrization basis for the functions describing the EFT of cosmic acceleration and, for the sake of simplicity, we focus on the Horndeski class of models.
%
...
The second one consists in re-parametrizing the couplings explicitly targeting the phenomenological features of Horndeski \cite{Bellini:2014fua}.
%
...
CMB-S4 measurements, in addition, are found to be the most constraining measurements on the other two EFT higher order operators, $\gamma_0^{(2)}$ and $\gamma_0^{(3)}$. Confirming the picture presented in the previous sections the synergy between CMB-S4 measurements and DESI results in much tighter constraints on all the considered parameters.
...
As in the constant EFT case the scalar field kineticity $\alpha_0^{\rm K}$ is found to be unconstrained.
