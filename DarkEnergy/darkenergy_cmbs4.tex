...
Noticeably when CMB-S4 and DESI are joined $\sigma_{8}$ gets constrained to $\sim 0.1\%$ at all times. Moreover, the gain in the joint constraint is higher than the gain in sensitivity in going from CMB-S4 to DESI, thanks to the cross correlation between the two surveys.
%
...
These levels of sensitivity will be instrumental in resolving tensions between different experiments. It is well known that CMB determinations of $\sigma_8$ are in marginal disagreement with LSS measurements. These tensions are still in a low statistical significance phase. Two experiments with similar, high, sensitivity can either confirm or falsify these tensions to high statistical significance making CMB-S4 and DESI instrumental to each other.

{\bf MR: here will add something more quantitative.}

This same trend appears also when considering the Growth index $\gamma_{L}$. CMB-S4 will give stronger constraints with respect to {\it Planck} due to the additional leverage of CMB lensing. These constraints will be comparable with DESI ones and displaying a slightly different degeneracy with the amplitude of scalar perturbations.
