...
We consider two parametrizations for the functions describing the EFT of cosmic acceleration and we shall restrict here to considering their Horndeski limit.
To parametrize theories in this class we shall adopt two basis. The first one is obtained by making the couplings in action (\ref{Eq:EFTaction}) dimensionless, as in \cite{Hu:2014oga}.
The second one consists of a re-parametrization of these couplings targeted at the class of Horndeski models, as in \cite{Bellini:2014fua}.
%
In both cases we consider two functional parametrizations.
The first one consists on assuming all the couplings constant in time. 
The second one allows to have an early time value for the EFT couplings and a late time value, with a smooth transition in between, inspired by \cite{Linder:2015rcz}. 
Specifically this second parametrization is given by $f(a)= 1/2 (f_0 + f_1) + ( f_1-f_0 ) {\rm ArcTan}[(a - a_0)/\Delta a]/\pi$ where 
$f_0$ and $f_1$ are respectively the early and late time values of the considered EFT function, $a_0$ is the transition scale factor assumed to correspond to $z=10$ and $\Delta a=0.01$ is the transition sharpness. 
%
