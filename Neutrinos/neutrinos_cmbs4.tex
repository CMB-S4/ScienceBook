...
\caption{ The effect of massive neutrinos on the matter power spectrum and CMB lensing power spectrum.  We see that only the overall suppression is visible at moderate to high-$\ell$ in lensing.}
...
Current best constraint on $\tau$ comes from Planck\cite{Adam:2016hgk}, roughly
corresponding to an
external prior of
...
\caption{ Forecasts for $\sigma(\sum m_\nu)$ assuming $\Lambda$CDM + $\sum m_\nu$ using CMB Stage IV and DESI BAO.  We vary sensitivity in $\mu$K-arcmin and $\tau$-priors, $\tau = 0.06 \pm \sigma(\tau)$ with the contours showing 1$\sigma$ errors for $\sum m_\nu$.  We fixed the resolution using 1' beams.  The white and blue dashed lines correspond to the low-$\ell$ cosmic variance limit and Planck Blue Book values respectively. }
\label{fig:mnu_tau}
\end{center}
\end{figure} 
...
we can reach $\sigma(\sum m_\nu) \sim 20$\,meV for a variety of plausible
...
available before or during CMB S4.  Possible impact of measurements in
even further future should also be examined in evaluating the value of
...
CLASS experiment is forecasted to reach
$\sigma(\tau) \simeq 0.004$\cite{CLASSSPIE2016}, for example.
%
Space missions\cite{2016JLTP..184..824M,2011JCAP...07..025K} during 2020s are
proposed to constrain the primordial gravitational
waves through the so-called reionization bump; they are designed to
reach sensitivity well beyond that required to achieve a cosmic-variance
limited $\tau$ measurement, $\sigma(\tau) \sim 0.002$.
%
Measurement of the reionization through 21-cm hyperfine transition can,
