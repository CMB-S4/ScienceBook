...
\caption{ The suppression in the matter power spectrum due to massive neutrinos. Plotted is the ratio of the matter power spectrum with $\sum m_\nu > 0$ to the matter power spectrum with $\sum m_\nu =0$ but the same value of $\Omega_m$. }
...
\includegraphics[scale=0.4]{Neutrinos/Mnu_tauprior.pdf}

\caption{ Forecasts for $\sigma(\sum m_\nu)$ assuming $\Lambda$CDM + $\sum m_\nu$ using CMB Stage IV and DESI BAO.  {\it Left:} Forecasts assuming a prior $\tau = 0.06 \pm 0.006$, corresponding to the Planck Blue Book expected sensitivity.  Hee we vary beam size in arcmin and effective detector noise in $\mu$K-arcmin {\it Right:} Forecasts for varying sensitivity in $\mu$K-arcmin and $\tau$-priors, $\tau = 0.06 \pm \sigma(\tau)$.  Contours show 1$\sigma$ errors for $\sum m_\nu$.  }
\label{fig:mnu_tau}
\end{center}
\end{figure} 
