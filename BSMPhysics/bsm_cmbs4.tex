...
\caption{Result derived using the CMB4cast framework~\cite{errard15b}, assuming a CMB-S4 observation with a single 150GHz channel, at $1\,\mu$K-arcmin ($1.4\,\mu$K-arcmin) in temperature (polarization), with a FWHM=3 arcmin. The instrument is assumed to observe $f_{\rm sky} = 40\%$, with $30\leq\ell\leq 4000$.
We consider no foregrounds, a white noise and a simple extension of the standard model: $\Lambda$CDM+$\Omega_K$. 
The algorithm uses informations from $TT$+$TE$+$EE$+$\kappa\kappa$ spectra, the lensing being characterized exploiting a $EB$ quadratic estimator. Cases with Planck use extra  $TT$+$TE$+$EE$ informations from Planck, with $30\leq\ell\leq 2500$ over an additional $20\%$ of sky. A prior on $\tau$ is used: $0.06\pm0.01$.}
