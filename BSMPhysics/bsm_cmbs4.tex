...
Fig.~\ref{fig:CB-forecast} shows the current upper limit (errorbar) on the rotation-angle power spectrum from POLARBEAR and Planck, and projections for an S4 experiment with noise of 1 $\mu$K-arcmin and resolution of 1'. The improvement from the current constraint at all multipoles is about two orders of magnitude. We assumed access to polarization modes from $\ell=30$ to $\ell=5000$.
\begin{figure}[h!]
\centering \includegraphics[width=0.70\textwidth]{BSMPhysics/birefringence-S4-planck-PB-v2.pdf}
\caption{Current best constraint on anisotropic birefringence will be improved by an S4 experiment by about two orders of magnitude at all multipoles. The errobar of the measurement of the rotation-angle power spectrum is shown on the vertical axis. For this forecast, we assumed noise of $1$ $\mu$K-arcmin and resolution of 1' and integrated polarization modes from $\ell=30$ to $\ell=5000$.}
\label{fig:CB-forecast}
\end{figure}

For a fixed integration time (and a varied noise level and sky coverage), large sky coverage optimizes sensitivity to low multipoles of the rotation angle and gives the best signal-to-noise ratio for rotation models that have most power on large scales (such as, for example, a model with a scale-invariant power spectrum, which could result from fluctuations in a spectator scalar field present during inflation). Conversely, for models that have power on scales below $\ell\sim 1000$, best signal-to-noise is achieved with deeper integration on small sky patches.  For a measurement of the magnitude of the quadrupole of rotation angle, reducing the resolution from 1' to 9' produces a factor of a few increase in the projected errorbar (for all other parameters fixed); increasing the noise from $1$ to $9$ $\mu$K-arcmin produces a factor of about $20$ increase in the errorbar. Access to polarization modes down to $\ell=2$ does not significantly affect the forecasts. 
