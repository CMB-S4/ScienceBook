...
Recent measurements by BICEP2/Keck/Planck \cite{bicepkeckplanck15} confirm that on degree scales, where 
...
Component separation aims at inverting Eq.~\ref{eq:comp_sep_data_modeling}, to estimate the foregrounds-disentangled CMB signal encapsulated in $s_p$, as well as the foreground templates which are relevant for testing and updating the sky model, as illustrated in Fig.~\ref{fig:general_comp_sep_scheme}.
...
the CMB contribution and the amplitudes of each foreground component (see, e.g., \cite{katayama11}).
\end{itemize}
...
%(see e.g. \cite{gold11,planck15-11} for real space and e.g \cite{bicepkeckplanck15} for Fourier space separations) 
...
%Nevertheless, when targeting a sensitivity equivalent to $r\simeq10^{-4}$, 
but these effects will become crucial at the sensitivity of \cmbexp\ and merit a dedicated study.
% has to be performed to choose and assess the most adapted approach.  
