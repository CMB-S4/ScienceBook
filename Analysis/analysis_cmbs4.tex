...
Beyond this basic data reduction, the full analysis pipeline (Figure \ref{fig_da}) also includes mission characterization and science exploitation branches. Time domain data are extensively used to build a model of the mission, comprising the instrument and the observation. For the instrument this can include such steps determining beam profiles from planet crossings, and estimating noise properties (including cross-correlations) from calibrated, signal-subtracted, detector data; for the observation, reconstructing a pointing solution from telescope sensor data and incorporating atmostpheric conditions in the data-flagging. The resulting mission model then feeds back into all of the ensuing data reduction and interpretation. The primary science exploitation derives cosmology and fundamental physics results from the various correlation functions of the CMB maps, from the power spectra's energy scale of inflation and neutino mass to the higher-order statistics'  measures of non-Gaussianity and the lensing potential. In addition the frequency maps provide significant astronomy and astrophysics observations, particularly when there are sufficient to isolate individual foreground components along with the CMB. 
